  ...
  context 'with unregistered user' do
    context 'with non public world' do
      it 'redirects to worlds path on access' do
        post world_facts_path(world)
        expect(response).to redirect_to new_user_session_path

        get new_world_fact_path(world)
        expect(response).to redirect_to new_user_session_path

        get edit_world_fact_path(world, fact)
        expect(response).to redirect_to new_user_session_path

        put world_fact_path(world, fact)
        expect(response).to redirect_to new_user_session_path

        get world_facts_path(world)
        expect(response).to redirect_to worlds_path

        get world_fact_path(world, fact)
        expect(response).to redirect_to worlds_path

        delete world_fact_path(world, fact)
        expect(response).to redirect_to new_user_session_path
      end
    end
  ...
  context 'with registerd user' do
    ...
    context 'owning world' do
      ...
      it 'shows all actions' do
        post world_facts_path(world, params: sample_params)
        expect(response).to have_http_status(:success)
        ...
        put world_fact_path(world, fact, params: sample_params)
        ...
        delete world_fact_path(world, fact)
        expect(response).to redirect_to world_facts_path(world)
      end
    end
